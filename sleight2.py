...
def convert_profile():
    ...
    if HTTPS == 'y':
        ...
        rules = (htaccess_template_https.format(uri,user_agent,LHOST,LPORT,domain))
    else:
        rules = (htaccess_template.format(uri,user_agent,LHOST,LPORT,domain))
...
def mod_rewrite_config(rules):
        ...
        subprocess.call(['a2enmod', 'rewrite', 'proxy', 'proxy_http', 'proxy_connect'])
        ...
        if 'https' in f[4]:
                ...
                subprocess.call(['a2enmod', 'proxy_connect'])
                ...
                </IfModule>'''.format(domain, domain, domain, domain, redirect, redirect)


                lessl = open('/etc/apache2/sites-enabled/000-default-le-ssl.conf', 'w')
                lessl.write(le_config)
                lessl.close()
